...
\title{MATH 341 / 650.3 Spring \the\year~Homework \#1}

\author{Professor Adam Kapelner} %STUDENTS: write your name here

\iftoggle{professormode}{
\date{Due by email, noon, February 11, \the\year \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\hardsubproblem{If $X \sim \bernoulli{\theta}$, find $\median{X}$. The median is defined as the 50\%ile which is the minimum realization element in the support that gives you at least 50\% of the total probability in the entire support.}\spc{2}

\intermediatesubproblem{If $X \sim \bernoulli{\theta}$, write the \qu{parametric statistical model} below using the notation we used in class only with a semicolon.}\spc{2}
...
\hardsubproblem{[MA] If $\Xoneton \iid f(x;\theta)$, prove why the log-likelihood of $\theta$ is normally distributed if $n$ gets large.}\spc{6}
...
\intermediatesubproblem{If $\Xoneton \iid \bernoulli{\theta}$, find the maximum likelihood estimator for $\theta$. An \qu{estimator} is a rv. Denote this $\thetahatmle$ and it should equal something that is a uppercase letter since capital letters denote rvs.}\spc{6}

\easysubproblem{If $\Xoneton \iid \bernoulli{\theta}$, find the maximum likelihood \textit{estimate} for $\theta$. An \qu{estimate} is a number, a real value. Denote this $\thetahathatmle$ and it should equal something that is a lowercase letter since lowercase letters denote rvs.}\spc{1}
...
\hardsubproblem{Why do frequentists have an insistence on $\theta$ being a fixed, immutable quantity? We didn't cover this in class explicitly but it is lurking behind the scenes. Use your reference resources.}\spc{4}

\easysubproblem{What are the three goals of inference? Give short explanations.}\spc{6}
...
\intermediatesubproblem{Why does some of frequentist inference break down if $n$ isn't large? Hint: property (2) from the previous question.}\spc{2}
...
\intermediatesubproblem{Explain why $\cprob{B}{A} \propto \cprob{A}{B}$. Hint: draw the shapes from class.}\spc{7}
